...
/**
 * Add-teacher modal for the school Teachers page. Creates a teacher at the
 * admin's own school (createTeacherAtSchool) and surfaces the one-time temp
 * password — shown once for the admin to copy.
 */

import { useActionState, useEffect, useRef, useState } from "react";
import { useRouter } from "next/navigation";
import { AlertCircle, Check, Copy, KeyRound, Loader2, X } from "lucide-react";
import { createTeacherAtSchool } from "@/lib/actions/school-users";
...
export function AddTeacherModal({ onClose }: { onClose: () => void }) {
  ...
  const [state, action, pending] = useActionState(
    createTeacherAtSchool,
    initialState
  );
  const firstFieldRef = useRef<HTMLInputElement>(null);
  const panelRef = useRef<HTMLDivElement>(null);
  const titleId = "add-teacher-title";
      ...
      >
        ...
        {state.success ? (
          ...
          <form action={action} className="space-y-3.5 px-5 py-5">
            ...
            <div>
              <label
                htmlFor="teacher_id_external"
                className="mb-1.5 block text-sm font-medium text-gray-700"
              >
                Teacher ID <span className="text-gray-500">(optional)</span>
              </label>
              <input
                id="teacher_id_external"
