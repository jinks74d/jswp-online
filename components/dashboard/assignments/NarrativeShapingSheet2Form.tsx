// components/dashboard/assignments/NarrativeShapingSheet2Form.tsx
"use client";
...
interface NarrativeShapingSheet2Data {
...
interface NarrativeShapingSheet2FormProps {
...
export default function NarrativeShapingSheet2Form({
  assignment,
  studentProfile,
}: NarrativeShapingSheet2FormProps) {
  const router = useRouter();
  const [shapingData, setShapingData] = useState<NarrativeShapingSheet2Data>({
    topicSentence: "",
    concreteDetails: [],
    commentary: "",
    concludingSentence: "",
  ...
  const debouncedAutoSave = useCallback(
    async (data: NarrativeShapingSheet2Data) => {
      ...
      try {
        ...
        const mergedData = {
          ...
          status: "shaping_sheet_2",
          working_on: "shaping_sheet_2",
        };
  ...
  const assembleColorCodedParagraph = useCallback((data: NarrativeShapingSheet2Data) => {
    const parts = [];
        ...
        if (response.ok && result.data) {
          if (result.data.concrete_details) {
            try {
              ...
              console.log("Loading Narrative Shaping Sheet 2 data:", stepData);

              // Load data from T-Chart 2
              let topicSentence = "";
              ...
              if (stepData.tChart2) {
                const tChart2 = stepData.tChart2;
                topicSentence = tChart2.topicSentence || "";
                
                if (tChart2.tChartData) {
                  const data = tChart2.tChartData;
                  const cds = [];
              ...
              const loadedData = {
                topicSentence: stepData.shapingSheet2?.topicSentence || topicSentence,
                concreteDetails: stepData.shapingSheet2?.concreteDetails || concreteDetails,
                commentary: stepData.shapingSheet2?.commentary || commentary,
                concludingSentence: stepData.shapingSheet2?.concludingSentence || concludingSentence,
                assembledParagraph: stepData.shapingSheet2?.assembledParagraph || "",
              };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(`✅ Body Paragraph 2 shaping sheet saved successfully!`);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Main Form - SHAPING SHEET FOR BODY PARAGRAPH 2 */}
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h2 className="text-2xl font-bold text-gray-900">
            SHAPING SHEET FOR BODY PARAGRAPH 2
          </h2>
          <button
        ...
        <div className="flex items-center gap-4 mt-8">
          ...
          <button
            ...
            className="px-6 py-3 bg-[#13161f] text-white rounded-lg hover:bg-gray-700 disabled:opacity-50 transition-colors font-medium"
          >
            {saving ? "Saving..." : "Save and Continue to T-Chart 3"}
          </button>
        </div>
      </div>
          ...
          <div className="bg-white rounded-lg p-6 max-w-2xl mx-4 max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold text-gray-900">
                BODY PARAGRAPH 2 SHAPING SHEET Tips
              </h3>
              <button
            ...
            <div className="space-y-4 text-gray-700">
              <div>
                <span className="font-semibold text-[#13161f]">1.</span> Review your topic sentence and ensure it transitions from paragraph 1.
              </div>
              <div>
                <span className="font-semibold text-[#13161f]">2.</span> Your concrete details should focus on the middle part of your narrative.
              </div>
              <div>
                <span className="font-semibold text-[#13161f]">3.</span> Commentary should explain the significance of these middle events.
              </div>
              <div>
                <span className="font-semibold text-[#13161f]">4.</span> The concluding sentence should bridge to your final paragraph.
              </div>
              <div>
                <span className="font-semibold text-[#13161f]">5.</span> Watch the right side preview to see how your paragraph is developing!
              </div>
            </div>
